...
public class Panel_unit : MonoBehaviour {
    ...
    public bool ActionFinished()
    {
        ...
        if (TailObj != null)
        {
            TrailRenderer trai = TailObj.GetComponentInChildren<TrailRenderer>();
            if (trai != null) {
                trai.time = 1.0f;
                trai.autodestruct = true;
            }
...
//            p.height = Config.UnitH;
    ...
    //====== Fight 
    public void ShowSkillTailFX(int nSkillID)
    {
        if (nSkillID == 0)
        {
            return;
        ...
        if (skl == null)
            return;
        FX fxData = ConstDataManager.Instance.GetRow<FX>(skl.n_TAIL_FX);
        if (fxData == null)
            return;
