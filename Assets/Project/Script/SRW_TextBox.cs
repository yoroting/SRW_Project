...
//    AddText("本遊戲的防禦系統較特別。每個角色身上都有防禦值，被攻擊時都會先扣除防禦值。\n當防禦值為零時才會真的扣除到生命值。生命值歸零則角色撤退不會真正死亡。\n防禦值的回復要靠同伴支援或防禦指令。同一回合內不受到任何攻擊則能全額回復。",3);
    ...
    void Update()
    {
        ...
        if (bIsTweenWidth) // bypass when tween width
        {
        ...
        //if( row != null )
        {
            ...
            if (tex)
            {
                if (tex != null)
                {
                    ...
                    tex.mainTexture = MyTool.GetCharTexture( CharID ) ;
                    ...
                    if (twH)
                    {
                        twH.from = 0;
                        twH.to = tex.height;
                        twH.SetOnFinished(OnTweenFaceEnd);
                        bIsTweenFace = true;
                    }
    ...
    public void SetDead()
    {
		TweenGrayLevel tw = GrayLevelHelper.StartTweenGrayLevel(this.gameObject, 2.0f , false );
        if (tw)
        {
            bIsDeading = true;
            tw.from = 0.0f;
    ...
    {
        bIsTweenWidth = false;
        //  OnTextBoxClick( _TextLineObj ); // auto click one line
        //if (--nTweenObjCount < 0)
        //{
        //    nTweenObjCount = 0;
        //}
